...
#        hd.print_text('It is the end of a wonderful year')
        ...
        text, world, select_grid, computer_turn = human_turn(
            text, world, select_grid, event)
...
def human_turn(text, world, select_grid, event):
    ...
    if hd.attack(event):
        world, computer_turn, text, select_grid = human_combat(
            select_grid, world, text)
...
def human_combat(select_grid, world, text):
    ...
    for i in range(n):
        ...
        if defender_pos != (-1, -1):
            ...
            logging.debug("Country " + repr(defender_pos) +
                          "is been attacked by player.")
            ...
            if combat(attacker_pos, defender_pos) and world[defender_pos] != attacker:
                ...
                logging.debug('Human player conquered ' + repr(defender_pos) +
                              '. It used to belong to' + repr(defender))
            else:
                computer_turn = True
